...
class FactorioCalculator:
    ...
    def get_item_info(self, item, recipe, assembly_setup, wanted_yield):
        ...
        for output in item_info.outputs:
            ...
            print("To make " + str(round(wanted_yield[item], 2)) + " " + item + " per sec, " +
    ...
    def calculate(self, item, wanted_yield):
        ...
        self.calculate_input_tree(items, wanted_yield_dict, info_list)

    def calculate_input_tree(self, items, wanted_yield, item_info_list):
        """
        ...
        for item in items:
            try:
                ...
                recipe['craft_time'] = 1 #TODO check! its 2 for uranium

            category = recipe['category']

            # TODO how to handle yield for multiple outputs? -- oil processing and stuff
            # category == crafting ... will never have two outputs??? TODO find out, not in vanilla at least?
            ...
            if item_info:  # TODO recursion stop condition, might change later
                ...
                self.calculate_input_tree(item_info.input_list, item_info.input_yield_ps, item_info_list)
...
def main():
    ...
    factorio_calc.calculate('automation-science-pack', 10)
